...
- **Auth lives in `providers/keycloak-provider.tsx`.** Consume it with
  ...
  `deploymentMode` ("standalone" | "saas" | `null` while loading) comes from the
  same call (backed by the public `GET /config`) — treat `null` the same as
  `"standalone"` in any new conditional (loading-safe default), don't gate on
  `=== "standalone"` alone. In standalone mode, `register()` aliases to
  `login()` — there's no self-serve signup, only admin-invited staff (see
...
- **Session cookies are chunked** (`crm_session.0`, `crm_session.1`, …). Browsers
  ...
  `lib/auth/session.ts`, never `cookies().set("crm_session", …)` directly. The
  `id_token` is deliberately kept in a separate `path=/api/auth/logout` cookie so
  it isn't re-sent on every request — but it must exist, or Keycloak shows a
  "Do you want to log out?" confirmation page instead of logging out silently.
