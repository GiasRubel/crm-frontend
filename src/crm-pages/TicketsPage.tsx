      ...
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500">Your support tickets and their status.</p>
          </div>
          <Button
            onClick={() => {
              form.reset();
        ...
        {message && (
          <div className="bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-xl p-4 flex items-center justify-between shadow-sm">
            <span className="text-sm font-medium flex items-center gap-2">
              <Check size={18} className="text-emerald-600" /> {message}
            </span>
        ...
        {errorMsg && (
          <div className="bg-red-50 border border-red-200 text-red-800 rounded-xl p-4 flex items-center justify-between shadow-sm">
            <span className="text-sm font-medium flex items-center gap-2">
              <AlertCircle size={18} className="text-red-600" /> {errorMsg}
            </span>
        ...
        <Card className="py-0 overflow-hidden">
          {myTicketsQuery.isLoading ? (
            ...
            <div className="flex flex-col items-center gap-2 text-gray-500 py-16">
              ...
              <p className="text-sm text-gray-400">Raise a ticket and we&apos;ll help you out.</p>
            </div>
          ) : (
            <ul className="divide-y divide-gray-100">
              {tickets.map((t) => (
                <li key={t.id}>
                  ...
                  >
                    <span className="text-xs font-mono font-bold text-gray-400 shrink-0">{t.number}</span>
            ...
            <>
              <DialogHeader>
                <DialogTitle className="flex items-center gap-2">
                  <span className="font-mono text-sm text-gray-400">{viewingFresh.number}</span>
              ...
              <div className="space-y-4">
                <div className="bg-gray-50/75 p-4 rounded-xl border border-gray-100">
                  <p className="text-sm text-gray-700 whitespace-pre-wrap">{viewingFresh.description}</p>
                </div>
    ...
    <div className="p-6 lg:p-8 bg-[#F4F5F7] min-h-screen">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          <StatCard label="Open" value={stats?.open} icon={Inbox} accent="bg-sky-50 text-sky-600" />
          <StatCard label="In Progress" value={stats?.inProgress} icon={Clock} accent="bg-indigo-50 text-indigo-600" />
          <StatCard label="Unassigned" value={stats?.unassigned} icon={UserRound} accent="bg-amber-50 text-amber-600" />
          <StatCard label="Urgent" value={stats?.urgent} icon={AlertTriangle} accent="bg-red-50 text-red-600" />
          <StatCard
                ...
                {ticketsQuery.isLoading ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={8} className="px-6 py-16 text-center">
                      <div className="flex flex-col items-center gap-2 text-gray-500">
                        <TicketCheck size={32} className="text-gray-300" />
                  ...
                  tickets.map((ticket) => (
                    <TableRow key={ticket.id} className="hover:bg-gray-50/50">
                      ...
                      <TableCell className="px-6 py-4 text-sm text-gray-600 truncate max-w-[160px]">
                        {ticket.customerName ?? "—"}
                      </TableCell>
                      ...
                      <TableCell className="px-6 py-4 text-sm">
                        {ticket.assignedToName ? (
                          <div className="flex flex-col min-w-0">
                            <span className="text-gray-700 font-medium truncate">{ticket.assignedToName}</span>
        ...
        <DialogContent className="max-w-2xl max-h-[90dvh] overflow-y-auto">
          {viewingTicket ? (
            <>
              <DialogHeader>
                <DialogTitle className="flex items-center gap-2">
                  <span className="font-mono text-sm text-gray-400">{viewingTicket.number}</span>
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {viewingTicket.firstResponseAt ? (
                    ...
                    <span className="text-[11px] text-orange-500 font-semibold">Awaiting first response</span>
                  )}
                </div>
                ...
                {viewingTicket.status !== "closed" && (
                  <div className="space-y-2">
                    <textarea
                      ...
                      className={cn(inputClasses, "resize-none", replyInternal && "bg-amber-50/50 border-amber-200")}
                      value={reply}
                      onChange={(e) => setReply(e.target.value)}
                    ...
                    <ul className="space-y-1">
                      {viewingTicket.relatedArticles.map((a) => (
                        <li key={a.id} className="text-sm text-gray-700 flex items-center gap-2">
                          <BookOpen size={13} className="text-gray-400 shrink-0" /> {a.title}
                        </li>
                      ))}
                    </ul>
                  )}
          ...
          {assigningTicket && (
            <>
              <DialogHeader>
                ...
                <DialogDescription>
                  Route <span className="font-semibold text-gray-700">{assigningTicket.number}</span> to a
            ...
            <>
              <DialogHeader>
                ...
                <DialogDescription>
                  ...
                  <span className="font-semibold text-gray-700">{deletingTicket.number}</span> and its
