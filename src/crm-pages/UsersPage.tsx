        ...
        <CardHeader className="pb-4">
          <div className="relative max-w-md group">
            ...
            <Input
              ...
              className="pl-10 bg-slate-50/50 border-slate-100 h-10"
            />
          </div>
        </CardHeader>
        <CardContent>
                ...
                {filteredUsers.length > 0 ? (
                  filteredUsers.map((user) => (
                    <TableRow key={user.id} className="group hover:bg-slate-50/30 transition-colors">
                      ...
                      <TableCell>
                        <Badge variant={user.active ? "outline" : "secondary"} className={user.active ? "text-emerald-600 bg-emerald-50 border-emerald-100" : "text-slate-400 bg-slate-50"}>
                          {user.active ? 'Active' : 'Inactive'}
                        </Badge>
                      </TableCell>
                      <TableCell className="text-slate-500 text-sm">{user.createdAt}</TableCell>
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="sm:max-w-[500px] rounded-2xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              <label className="text-sm font-semibold text-slate-700">Team Role</label>
                ...
                {['admin', 'sales', 'support'].map((role) => (
                  <Button
                    ...
                    className={cn(
                      ...
                      formData.role === role ? "shadow-sm" : "text-slate-500"
                    )}
