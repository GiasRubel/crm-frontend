        ...
        <div className="max-w-3xl mx-auto px-6 py-12 text-center space-y-4">
          ...
          <div className="relative max-w-xl mx-auto">
            ...
            <Input
              ...
              className="pl-10 bg-white text-gray-900 h-11"
            />
          </div>
        </div>
      </div>
        ...
        {openSlug ? (
          ...
          <Card>
            <CardContent className="p-8 space-y-5">
              <Button variant="ghost" size="sm" className="text-gray-500 -ml-2" onClick={() => setOpenSlug(null)}>
              ...
              {articleQuery.isLoading ? (
                ...
                <p className="text-sm text-gray-500 py-8 text-center">This article is no longer available.</p>
              ) : (
                <>
                  <div>
                    <h2 className="text-2xl font-bold text-gray-900">{article.title}</h2>
                    {article.category && (
                      <Badge variant="outline" className="mt-2 bg-slate-50 text-slate-600 border-slate-200 capitalize">
                        {article.category}
                      </Badge>
                    )}
                  </div>
                  <p className="text-[15px] leading-7 text-gray-700 whitespace-pre-wrap">{article.body}</p>
                  {article.tags.length > 0 && (
                    <div className="flex gap-1.5 flex-wrap">
                      {article.tags.map((t) => (
                        <Badge key={t} variant="outline" className="bg-white text-gray-400 border-gray-200">
                          #{t}
                        </Badge>
                      ))}
                    </div>
                  ...
                  <div className="border-t border-gray-100 pt-5 flex items-center gap-3">
                    ...
                    {voted[article.id] ? (
                      <span className="text-sm font-medium text-emerald-600">Thanks for the feedback!</span>
                    ) : (
            ...
            {listQuery.isLoading ? (
              ...
              grouped.map(([cat, items]) => (
                <div key={cat} className="space-y-2">
                  <h3 className="text-xs font-bold text-gray-400 uppercase tracking-[2px] px-1 capitalize">
                    {cat}
                  </h3>
                      ...
                      {items.map((a) => (
                        <li key={a.id}>
                          ...
                          >
                            ...
                            <span className="font-medium text-gray-800 flex-1 truncate">{a.title}</span>
                            {a.helpfulCount > 0 && (
