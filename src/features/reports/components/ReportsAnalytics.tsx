...
function ChartCard({
  ...
  return (
    <div className={cn("bg-white rounded-2xl shadow-sm p-6 flex flex-col", className)}>
      <div className="mb-4">
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        <StatCard
          ...
          accent="bg-[#3F51B5]/10 text-[#3F51B5]"
        />
        <StatCard
        ...
        <ChartCard title="Pipeline by Stage" subtitle="Deal value per stage" className="h-[340px] lg:col-span-2">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={pipelineData} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#F0F0F0" />
              <XAxis dataKey="stage" axisLine={false} tickLine={false} tick={{ fill: "#9CA3AF", fontSize: 11 }} />
              <YAxis
                axisLine={false}
                tickLine={false}
                tick={{ fill: "#9CA3AF", fontSize: 12 }}
        ...
        {teamQuery.isLoading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {team.teams.map((t) => (
                  <tr key={t.teamId} className="border-b border-gray-50 last:border-0">
                    <td className="py-2.5 pr-4 font-medium text-gray-800">{t.teamName}</td>
                    <td className="py-2.5 px-4 text-right text-gray-500">{t.memberCount}</td>
                    <td className="py-2.5 px-4 text-right text-gray-600">{t.wonCount}</td>
                    ...
                    <td className="py-2.5 pl-4 text-right font-semibold text-gray-800">{t.winRate}%</td>
                  </tr>
                ))}
              </tbody>
            </table>
